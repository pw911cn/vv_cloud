...
/*				del,
		...
		prototype: {
			...
			render:function(data,isnew){
				...
				if(data){
					var render_data=$.isArray(data)?data:[data];
					var $html=$(template.render(tpl,{data:render_data,key:config.key,edit:config.edit,name:config.name,hidevalue:config.hidevalue,widthopt:config.widthopt}));
					if(isnew){
			...
			bindevent:function(){
				...
				el.on("click","."+config.optClass.wrap,function(){
					...
					if(config.callbacks.wrapclick){
						config.callbacks.wrapclick(data,wrap,me);
					}
				});
				el.on("click","."+config.optClass.operate,function(ev){
					ev.stopPropagation();
					var index=$(this).parents("."+config.optClass.wrap).index();
					var wrap=$(this).parents("."+config.optClass.wrap);
					el.find("."+config.optClass.wrap).removeClass("active");
					el.find("."+config.optClass.wrap).eq(index).addClass("active");
					var data=me.data[index];
					if(config.callbacks.operate){
						config.callbacks.operate(data,wrap,me);
					}
				});
