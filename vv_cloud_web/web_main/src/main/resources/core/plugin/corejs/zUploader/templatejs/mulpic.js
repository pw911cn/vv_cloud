define(["jquery","templateweb","rebox","text!../template/mulpic.html"], function ($,template,rebox,mulTpl) {
    ...
    function start(file,opts,uploader,container){
    	...
    	if(item.length==0){
        	...
        	if(!opts.resulttag){
        		opts.resulttag="<div style='width:300px;height:200px' abc=1></div>"
        	}
    ...
    function bindBtns(file,opts,container,uploader,state) {
        ...
        if(state && state == 'uploadStart'){
            $file.find('.operate_delete.vert').off().on('click', function(){
                try {
