define([ 'jquery', 'corectr','WdatePicker'], function($, corectr,WdatePicker) {
	...
	var _eventbind = function($thispage, renderdata, opts) {
		$("#backlog_list_search",$thispage).on("click",function(){
			var param=$("#backlog_list_searchform").getformdata();
			var data={};
			data.param=param;
			data.pageVo={
				pageNumber:1
		...
		$("#backlog_list_reset",$thispage).on("click",function(){
			if (!!$('form[id="backlog_list_searchform"]')[0]){
				$('form[id="backlog_list_searchform"]')[0].reset();
			}
		});
	};
    ...
    function tableInit($thispage,data) {
    	var table = corectr.v.table({
    		// 承载表格的容器
    		el: $("#backlog_list",$thispage),
    		// 定义表格列
    		columns: [
    			{
    				title: "状态", width: 140, scale: 0, align: "center",
    			...
    			{
    				title: "会诊方式", width: 160,
    				align: "center",
    				scale: 0,
    				render: "rowData.consType_name"
    			},
    			{
    				title: "患者姓名", width: 140, scale: 0,
    				align: "center",
    				render: "rowData.pati_name"
    			},
//    			{
//    				title: "会诊来源", width: 130, scale: 0,
//    				align: "center",
//    				render: "rowData.platform_name"
//    			},
    			{
    				title: "申请医院", width: 260, scale: 0,
    				align: "center",
    			...
    			{
    				title: "会诊时间及地点", width: 200, scale: 0,
    				align: "center",
    			...
    			
    			{
    				...
    				render: function(data,table){
                        ...
                        $btn1.on('click',function () {
                        	corectr.loadmodule({
                    			module : "web_cons/cons_apply/apply_light/apply_light",
                    			$todom : $(".tms4SubIndex_mainData_lyt", $thispage),
                    			initdata_param :{"prim_id":data.rowData.prim_id,"prim_status":data.rowData.prim_status}
                    		});
                        });
                        $btn1.appendTo(data.$td);
                        var $btn2 = $("<button class='xBtn xBtn-default' type='button'>补充资料</button>");
                        $btn2.on('click',function () {
                      	   alert("补充资料按钮"+"prim_id:"+data.rowData.prim_id+","+"prim_status:"+data.rowData.prim_status);
                         });
                        $btn2.appendTo(data.$td);
                    }
    			}
    		],
    		// 有分页
    		...
    		ajax: {url: "http://localhost:8080/server_cons/consApplyQuery/toDoList", data:{param:data.param}}
    	});
    }
