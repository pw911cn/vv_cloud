define([ 'jquery', 'corectr','WdatePicker'], function($, corectr,WdatePicker) {
	...
	var _eventbind = function($thispage, renderdata, opts) {
		$("#backlog_conference_list_search",$thispage).on("click",function(){
			var param=$("#backlog_conference_searchform").getformdata();
			var data={};
			data.param=param;
			data.pageVo={
				pageNumber:1
		...
		$("#backlog_conference_list_reset",$thispage).on("click",function(){
			if (!!$('form[id="backlog_conference_searchform"]')[0]){
				$('form[id="backlog_conference_searchform"]')[0].reset();
			}
		});
	};
    ...
    function tableInit($thispage,data) {
    	var table = corectr.v.table({
    		// 承载表格的容器
    		el: $("#backlog_conference_list",$thispage),
    		// 定义表格列
    		columns: [
    			{
...
//    			},
    		...
    		ajax: {url: "http://localhost:8080/server_cons/consApplyQuery/conferenceList", data:{param:data.param}}
    	});
    }
