define([ 'jquery', 'corectr'], function($, corectr) {
		...
		$("#role_list_addBtn",$thispage).on('click',function() {
			corectr.v.popup({
                ...
                content:function($wrap, instance){
                	corectr.loadmodule({ 
                		module : "web_base/role_perm/role_perm_add/role_perm_add",
                		$todom : $wrap,
                		vpopup:instance,
                        renderdata:{
                            menuId:menuId
	...
	function choose(role_id){
		corectr.v.popup({
            ...
            content:function($wrap, instance){
            	corectr.loadmodule({ 
            		module : "web_base/role_perm/role_perm_choose/role_perm_choose",
            		$todom : $wrap,
            		vpopup:instance,
            		renderdata:{
            				role_id:role_id,
	...
	function role_edit(role_id){
    	corectr.v.popup({
            ...
            content:function($wrap, instance){ 
            	corectr.loadmodule({ 
            		module : "web_base/role_perm/role_perm_edit/role_perm_edit",
            		$todom : $wrap,
            		vpopup:instance,
            		initdata_param:{
            			param:{
            		...
            		initdata_url:sys_config.base_serverpath+"/power/queryOneRole"
            	});
            },
            closeCallback:function($wrap, instance){
            	if(instance.data("closeType")&&instance.data("closeType")=="submit"){
	...
	function role_toggle_freeze(flag,role_id){
		var confirmIns =corectr.v.vConfirm({
			...
			confirmHandler: function () {
				  ...
				  $.ajax({
						type :"post",					 
						url : sys_config.base_serverpath+"/power/editPowerRole",
	...
	function assuser(role_id){
		corectr.v.popup({
            ...
            content:function($wrap, instance){ 
            	corectr.loadmodule({ 
            		module : "web_base/role_perm/role_perm_assuser/role_perm_assuser",
            		$todom : $wrap,
            		vpopup:instance,
            		renderdata:{
            				role_id:role_id,
    ...
    function tableInit($thispage,data) {
    	if(!table){
            ...
            table =corectr.v.table({
                ...
                ajax: {url: sys_config.base_serverpath+"/power/roleList",
