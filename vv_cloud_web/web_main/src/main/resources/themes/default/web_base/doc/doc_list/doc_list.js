define([ 'jquery', 'corectr'], function($, corectr) {
	...
	function doc_edit(account_id){
    	corectr.v.popup({
            ...
            content:function($wrap, instance){ 
            	corectr.loadmodule({ 
            		module : "web_base/doc/doc_edit/doc_edit",
            		$todom : $wrap,
            		vpopup:instance,
            		initdata_param:{
            			param:{
            		...
            		initdata_url:sys_config.base_serverpath+"/base_user_doctor/getBaseUserDoctorById",
	...
	function doc_toggle_freeze(flag,account_id){
		var confirmIns =corectr.v.vConfirm({
			...
			confirmHandler: function () {
				  ...
				  $.ajax({				 
						url : sys_config.base_serverpath+"/base_user_doctor/del",
		...
		var confirmIns = corectr.v.vConfirm({
			...
			confirmHandler: function () {
				var data={param:{}};
				  data.param.account_id=account_id;
				  $.ajax({				 
						url : sys_config.base_serverpath+"/base_doc_gov/del",
						data:JSON.stringify(data),
				 		success : function(result) {
				 			corectr.v.message.success({content:result.message});
				 		},
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function(data,table){
                            ...
                            if(data.rowData.flag == '1'){
                                //冻结
                                $btn2 = $("<button class='xBtn xBtn-warning' type='button'>冻结账号</button>");
                            }else if(data.rowData.flag == '2'){
                                //解冻
                                $btn2 = $("<button class='xBtn xBtn-success' type='button'>解冻账号</button>");
                            }
    ...
    /*                        var $btn3 = $("<button class='xBtn xBtn-danger' type='button'>删除</button>");
                ...
                ajax: {
                	url: sys_config.base_serverpath+"/base_user_doctor/list",
