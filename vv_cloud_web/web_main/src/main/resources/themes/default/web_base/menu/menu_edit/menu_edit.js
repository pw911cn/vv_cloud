define([ 'jquery', 'corectr'], function($, corectr) {
	...
	var _viewinit = function($thispage, renderdata) {
		...
		$("#fuzzy_menu_id",$thispage).fuzzySearch({
			url: sys_config.base_serverpath+"/menu/dimQuery",
			key:"menu_name",
			data: function(){
				return {param:{
	...
	var _eventbind = function($thispage, renderdata, opts) {
		  $thispage.initSelect();
		  $("#menu_edit_save",$thispage).on("click",function(){
			  if(!$("#menu_edit_form").valid()){
				  return
			  }
			  var data={};
			  ...
			  $.ajax({
					type :"post",					 
					url : sys_config.base_serverpath+"/menu/edit",
		  ...
		  $("#menu_edit_cancle",$thispage).on("click",function(){
				opts.vpopup.remove();      
		  });
	};
