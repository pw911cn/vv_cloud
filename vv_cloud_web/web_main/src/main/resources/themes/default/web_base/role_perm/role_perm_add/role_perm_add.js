define([ 'jquery', 'corectr','plugin/corejs/zTreeSearch/zTreeSearch'], function($, corectr,zTreeSearch) {
				...
				$(".choose_list",$thispage).activeTag({
					...
					callbacks:{
						operate:function(data){
							queryAllianceOrg($thispage,renderdata,data.id,data.org);
						},
	...
	var _eventbind = function($thispage, renderdata, opts) {
		  $("#role_add_save",$thispage).on("click",function(){
			  ...
			  $.ajax({
					type :"post",					 
					url : sys_config.base_serverpath+"/power/addRole",
					data:JSON.stringify(data),
	...
	function queryAllianceOrg($thispage,renderdata,menuid,orgids){
		var orgdata={param:{
		...
		$.ajax({
			...
			success:function(data){
				...
				if(targetarr.length>1){
					...
					$.each(targetarr,function(i,n){
						if(("all_org_flag" in n)&&n.all_org_flag=="1"){
							$(".myhos [name='all_org_flag']",$thispage).prop("checked",true);
						}else{
							$(".myhos [name='all_org_flag']",$thispage).prop("checked",false);
							n.all_org_flag="0"
						}
						if(("all_detp_flag" in n)&&n.all_detp_flag=="1"){
							$(".myhos [name='all_detp_flag']",$thispage).prop("checked",true);
						}else{
							$(".myhos [name='all_detp_flag']",$thispage).prop("checked",false);
							n.all_detp_flag="0"
						}
