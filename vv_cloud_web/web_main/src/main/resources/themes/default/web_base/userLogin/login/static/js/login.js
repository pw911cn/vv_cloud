...
_loginBtn.onclick = function () {
	if(document.querySelector('#WisdomPlatform')){
	    ...
	    $.ajax({
			url: config.server_zuulpath+"/auth-server/auth2.0/login?client_id="+config.client_id+"&client_secret="+config.client_secret+"&scope="+config.scope+"&grant_type="+config.grant_type+"&redirect_uri="+config.redirect_uri+"&username="+username+"&password="+password,
			type : "GET",
			dataType : 'json',
			async : true,
			data : "",
			    ...
			    */
			    ...
			    var waitTimerFunC = setTimeout(function () {
			        ...
			        if(Login_Status){
			    		...
			    		//登录成功
			            Login_Ok_FunC(result.access_token);
			        }else {
			        	//登录失败
			            Login_Error_FunC();
			        }
...
function Login_Ok_FunC(token) {
    ...
    window.location.href = "http://localhost/index.htm?token=" + token + "#user_portal/home/home";
}
