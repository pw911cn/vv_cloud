...
_loginBtn.onclick = function () {
	if(document.querySelector('#WisdomPlatform')){
	    ...
	    //password = '123456';
	    $.ajax({
			url: sys_config.zuul_serverpath+"/auth-server/auth2.0/login?client_id="+sys_config.client_id+"&client_secret="+sys_config.client_secret+"&scope="+sys_config.scope+"&grant_type="+sys_config.grant_type+"&redirect_uri="+sys_config.redirect_uri+"&username="+username+"&password="+password,
			type : "GET",
			dataType : 'json',
			//async : true,
			data : "",
			    ...
			    */
			    ...
			    var waitTimerFunC = setTimeout(function () {
			        ...
			        if(Login_Status){
			    		...
			    		//登录成功
			            Login_Ok_FunC(result.datas.access_token);
			        }else {
			        	//登录失败
			            Login_Error_FunC();
			        }
...
function Login_Ok_FunC(token) {
    ...
    if(location.hash!=""&&location.hash!="#"){
    	window.location.href = "../../index.html?token="+token+location.hash;;
    	return;
    }
    window.location.href = "../../index.html?token=" + token + "#user_portal/home/home";
}
...
function getconfig(){
	...
	$.ajax({
		type:"get",
		url:"../../config.json?"+new Date().getTime(),
		dataType:"json",
		async:false,
