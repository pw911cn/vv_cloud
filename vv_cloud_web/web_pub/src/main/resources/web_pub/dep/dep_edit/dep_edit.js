define([ 'jquery', 'corectr'], function($, corectr) {
	...
	var _viewinit = function($thispage, renderdata) {
		...
		console.log($thispage.find("select[name='dept_level'] option[value='"+renderdata.datas.dept_level+"']"))
		$thispage.find("select[name='dept_level'] option[value='"+renderdata.datas.dept_level+"']").prop("selected",true);
		$thispage.find("select[name='special_id'] option[value='"+renderdata.datas.special_id+"']").prop("selected",true);
		$thispage.find("input[name='is_admin'][value='"+renderdata.datas.is_admin+"']").prop("checked",true);
		
		
		if($thispage.find("input[name='hosp_name']").val().length){
	...
	var _eventbind = function($thispage, renderdata, opts) {
		  ...
		  $("#dep_edit_save",$thispage).on("click",function(){
			  var data={};
			  data.param=$thispage.getformdata();
			  $.ajax({
					type :"post",					 
					url : server_zuulpath+"/base_dept/save",
					data:JSON.stringify(data),
		  ...
		  $("#dep_edit_cancle",$thispage).on("click",function(){
				opts.vpopup.remove();      
		  });
	};
